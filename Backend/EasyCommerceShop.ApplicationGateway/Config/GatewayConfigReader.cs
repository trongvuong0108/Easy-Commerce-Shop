namespace EasyCommerceShop.ApplicationGateway.Config
{
    public static class GatewayConfigReader
    {
        public static GatewayConfig GetGatewayConfig(this IConfiguration config)
        {
            var result = new GatewayConfig
            {
                ...
                ApiPath = config.GetValue("Gateway:ApiPath", "/api/") ?? "/api/",
                ...
                DownStreamServices = config.GetSection("OpenIdConnect:DownStreamServices").Get<List<DownStreamServiceConfig>>() ?? new List<DownStreamServiceConfig>()
            };
